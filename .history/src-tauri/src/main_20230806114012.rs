...
use std::{sync::Mutex, collections::HashMap, thread};

pub struct DictionaryState<'static >(Mutex<Dictionary>);
#[derive(Default)]
...
// TODO: Optimisation
#[tauri::command]
fn new_dictionary(state: tauri::State<'static, DictionaryState>) {
    ...
    thread::spawn(move || {
        ...
        *state.0.lock().unwrap() = Dictionary {
            map: Some(hashmap)
        };
    });
}
