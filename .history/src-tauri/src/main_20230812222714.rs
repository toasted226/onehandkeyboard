...
#[tauri::command]
async fn set_layout(state: tauri::State<'_, ConfigState>, layout_id: i32) -> Result<(), String> {
    ...
    if layout.is_some() {
        state.0.lock().unwrap().layout = layout.unwrap();
    }

    match config_setup(state).await {
        Ok(_) => Ok(()),
        Err(_) => Err(String::from("Failed to setup config"))
    }
}
