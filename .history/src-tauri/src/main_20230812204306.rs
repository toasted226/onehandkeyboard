...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![new_dictionary, on_text_change, letter_to_symbol, config_setup, first_time_startup])
...
async fn new_dictionary(state: tauri::State<'_, ConfigState>) -> Result<(), String> {
    ...
    let layout = state.0.lock().unwrap().layout;
    state.0.lock().unwrap().map = Some(onehandkeyboard::create_hashmap(&words, &state.0.lock().unwrap().layout));
    Ok(())
}
...
#[tauri::command]
async fn config_setup(state: tauri::State<'_, ConfigState>) -> Result<(), String> {
    state.0.lock().unwrap().layout = KeyboardLayout::Dvorak;
    Ok(())
}

#[tauri::command]
async fn first_time_startup(app: tauri::AppHandle<>, state: tauri::State<'_, ConfigState>) -> Result<bool, String> {
